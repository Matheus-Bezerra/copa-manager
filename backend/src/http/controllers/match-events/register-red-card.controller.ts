...
import { RegisterRedCardUseCase } from '@/use-cases/match-events/register-red-card'
import { formatError } from '@/utils/errors/format-error'
import { logger } from '@/utils/logger'

export async function registerRedCardController(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    ...
    const useCase = new RegisterRedCardUseCase(
    ...
    logger.error('Register red card error', err)
