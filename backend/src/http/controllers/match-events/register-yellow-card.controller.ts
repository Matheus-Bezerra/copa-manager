...
import { RegisterYellowCardUseCase } from '@/use-cases/match-events/register-yellow-card'
import { formatError } from '@/utils/errors/format-error'
import { logger } from '@/utils/logger'

export async function registerYellowCardController(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    ...
    const useCase = new RegisterYellowCardUseCase(
    ...
    logger.error('Register yellow card error', err)
